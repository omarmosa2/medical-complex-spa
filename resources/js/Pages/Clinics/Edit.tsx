...
export default function Edit({ auth, clinic }: PageProps<{ clinic: any }>) {
    // Initialize schedules from existing data or defaults
    const initialSchedules = days.map((_, index) => {
        const existing = clinic.schedules?.find(s => s.day_of_week === index + 1);
        return existing ? {
            ...existing,
    ...
    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">تعديل العيادة</h2>}
        >
            <Head title="تعديل العيادة" />
